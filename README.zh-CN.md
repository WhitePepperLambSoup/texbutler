...
3. **中文 LaTeX 特有问题检查** — 10 条规则：裸 `%`、`\textit` 包中文、`[ht]` 浮动错位、浮点垃圾、段落粘连等，保存即自动检查。
...
5. **流畅写作流（v0.3.0）** — 图片拖拽进编辑器 / 截图直接粘贴（自动保存并弹出宽度/位置/图注选项）、**大纲面板**（章节树点击跳转）、**参考文献面板**（点击 .bib 条目插入 `\cite`）、LaTeX 自动补全（60+ 命令、环境配对）、可选的保存后自动编译、启动会话恢复、Ctrl+P 快速打开文件。
6. **端到端可验证** — 真实 API + 真实编译的端到端测试（`cargo test --test e2e_ai -- --ignored`），修复闭环每一步都有实测兜底。
7. **液态玻璃界面（v0.4.0）** — 默认玻璃拟态主题：动画渐变光斑 + 磨砂玻璃面板 + 高光按钮；三套外观（液态玻璃 / 经典深色 / 经典浅色）工具栏一键切换并持久化，Monaco 编辑器主题联动。Monaco 本地打包完全离线，不再依赖 CDN；工具栏常驻 12 个快捷公式符号（α β γ δ θ λ π √ ∫ ∞ ± ≤）+ 90 个符号面板。
8. **悬空引用规则 + 智能补全（v0.5.0）** — 规则引擎第 10 条：编译前检查每个 `\ref` 是否匹配项目内 `\label`、每个 `\cite` 是否在 bib 中；`\ref{` / `\cite{` 从实时项目索引自动补全。另有：状态栏字数统计（排除注释/命令）、可视化表格生成器、保持 LaTeX 结构的 AI 翻译、SyncTeX 正向搜索（定位到 PDF）、多主文档编译目标、LaTeX → Markdown/Word 导出、保存后防抖自动编译。
9. **AI 协同编辑（v0.6.0）** — 直接在 AI 面板提出修改要求（"把标题改成…""每个 Question 前加分页"），AI 以声明式工具调用（插入/替换/删除）精确改写文件，程序确定性执行——批量一次应用、失败自动重试，不再有脆弱的 diff 解析。每次修改都有快照可回滚，并即时同步进编辑器与 PDF 预览（编译验证闭环：AI 改完自动重新编译，失败自愈一轮）。AI 知道你的文档（文档类/宏包/中文支持/编译引擎）、记得对话上下文，且从不覆盖你未保存的输入。面板为右侧伸缩式竖条（收起 34px），支持流式回答、选区提问、token 用量统计、项目级 `AI_GUIDE.md` 指南注入。
10. **模板市场与文件工作流（v0.7.0）** — 160+ 个已验证模板，覆盖国内 985/211、海外 QS 前 100、期刊、幻灯片、海报、简历、信件、报告与课程作业；8 个内置模板构建时真机编译验证，下载模板通过结构检查（`\documentclass` 主文件检测）后才可使用。**新建文件在当前目录创建并可选模板**（article/ctexart/report/beamer/minimal/空白）；模板也可安全导入已打开的项目（冲突检测、失败回滚与残留清理）。AI 会话**按项目文件持久化**并随编辑器标签自动切换；AI 面板布局重建（消息区/输入区/工具菜单不再互相遮挡），编辑器工具在窄窗口自动收进溢出菜单，所有面板分隔条可可靠拖拽。
11. **欢迎页与项目仪表盘（v0.7.0）** — 启动时欢迎页列出最近打开的项目一键恢复（失败打开自动清理）；状态栏显示每个项目的累计编译次数与字数历史。
...
5. "规则检查"页签展示 10 条中文 LaTeX 规则命中（含修复提示与一键确定性修复），可在设置中逐条开关；
6. 底部状态栏显示引擎/耗时/问题数；设置中可查看系统字体检测与 bundle 状态。

### v0.2.0 写作辅助

- **插图**：编辑器工具栏图片按钮选图 → 自动复制进项目并在光标处生成 `figure`/`includegraphics` 代码。
- **快速格式**：工具栏按钮（段落/章节/粗体/行内与行间公式/列表/表格）。
- **AI 生成**：AI 面板输入自然语言（如"生成一个 booktabs 三线表"）→ AI 直接写入当前文件（或作为回复返回代码），一键回滚。
- **Word 导入**：工具栏 **Word→LaTeX** → 选 `.docx` → 解析标题/段落/表格 → AI 自动生成完整可编译 LaTeX。
- **模板库**：项目树星标按钮将当前项目存为模板；新建项目可选内置 + 用户模板（用户模板可删除）。
- **数学符号**：αβ 按钮打开 36 个符号面板（α β γ … ∑ ∫ √ ± ≤ ≥ ≈ ≠ ∈ ∀ ∃）——点击即键入，无需背命令。
- **日夜主题**：工具栏日间/夜间一键切换，重启后记忆。
...
├── assets/e2e/           # 端到端测试样例
├── assets/screenshot-project/ # README 截图用展示项目
...
规则库通过注册表扩展：新增规则只需在 `src-tauri/src/core/rules/` 加文件并在 `all_rules()` 注册一行。多数命中提供一键确定性修复（如段落粘连可批量修完，无需 AI）。

## AI 修复闭环（架构）

1. **确定性修复**（不依赖 AI）：缺 xcolor → 自动加宏包；独立成行的未定义命令 → 按编译器行号精确删除；缺 `\end{document}` → 自动补；段落粘连 → 一键批量修复（无需 AI）
...
AI 面板的对话修改走同一套加固管线，但改用**声明式工具调用**而非自由格式 diff：`insert_before` / `insert_after` / `replace` / `delete_line`（锚文本匹配、强制唯一、行号可选）。程序确定性执行——批量调用按文件原子应用、每个文件独立快照与回滚、应用失败自动用最新文件内容重试一次；自由格式 diff 保留为兜底。每次编辑后 AI 自动重新编译，失败时错误回喂自愈一轮；结果（编辑器 + PDF 预览 + 诊断面板）自动同步——除了你正在输入（未保存）的页签。
...
安全约定：AI 请求只发送错误片段 + 局部上下文（前后各 20 行，第 2 轮起附加完整文件），**绝不发送其他文件**；api_key 仅存本机、不写入日志；修复永远先预览 diff，任何修改都有快照可回滚。编辑仅限 `.tex/.bib/.sty/.cls` 白名单（`AI_GUIDE.md` 与 `.texbutler` 受保护）、路径归一化防穿越、指南注入忽略行为指令尝试。
