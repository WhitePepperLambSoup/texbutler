...
9. **AI co-editing (v0.6.0)** — chat with the AI and it edits your files: ask in plain language ("change the title to…", "add a three-line table") and the AI applies a minimal diff automatically, snapshotted first so you can roll it back from the message bubble after compiling. The AI panel is a collapsible right-side rail (Overleaf style, 34px strip when closed). Streaming replies, selection-based questions, session token usage with cost estimate, and a per-project `AI_GUIDE.md` style guide generated from your requirements and injected into every AI prompt.
