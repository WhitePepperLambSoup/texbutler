...
- **液态玻璃主题（默认）**：动画渐变光斑画布 + 磨砂玻璃面板（backdrop-filter 模糊/饱和）+ specular 高光按钮 + 渐变品牌字 + SVG 噪声质感；编辑器使用深色半透明面板（编辑区不 blur，保证打字流畅）。
- **三主题外观切换**：工具栏色块选择器，液态玻璃 / 经典深色 / 经典浅色一键切换并持久化；Monaco 编辑器主题联动（液态玻璃用透明背景变体）。
